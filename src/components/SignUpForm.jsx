            ...
            >
                {({ errors, touched }) => (
                    <Flex flexDirection={"column"} gap={"10px"} as={Form} maxWidth="100%" >
                        ...
                        <Input as={Field} variant="login" id="firstName" name="firstName" type="text" placeholder="Nombre" />
                        {errors.firstName && touched.firstName && <label htmlFor="firstName">{errors.firstName}</label>}
                        <Input as={Field} variant="login" id="lastName" name="lastName" type="text" placeholder="Apellido" />
                        {errors.lastName && touched.lastName && <label htmlFor="lastName">{errors.lastName}</label>}
                        <Input as={Field} variant="login" id="email" name="email" type="email" placeholder="Email" />
                        {errors.email && touched.email && <label htmlFor="email">{errors.email}</label>}
                        <Input as={Field} variant="login" id="password" name="password" type="password" placeholder="Contraseña" autoComplete="new-password" />
                        {errors.password && touched.password && <label htmlFor="password">{errors.password}</label>}
                        <Button variant={"login"} type="submit">Registrarme</Button>
                        <Flex justifyContent="center" gap="4px">
                            <Text>¿Ya tienes una cuenta?</Text>
                            <Text color="primary" _hover={{ cursor: "pointer" }} onClick={() => navigate("/login")}>Iniciar Sesión</Text>
                        </Flex>
                    </Flex>
                )}
            </Formik>
